...
Каждый вызов скилла оставляет строку в `[todo · incidents]/ai.incidents.md` → таблица `## Append-only trace`:
...
Сохранять в `[todo · incidents]/reasoning-logs/{date}-{skill}-{short-id}.md`:
