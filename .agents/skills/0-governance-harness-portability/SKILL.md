...
- **Канон guardian-скила = этот skill** (`0-governance-harness-portability`); hook — тонкий cycle-end вызов, НЕ новый
  verifier. Состав/lifecycle/project-слой харнесса — единый SSOT `harness-workflow.yaml`
  (canonical `0-heroes-harness-mirror-guardian` живёт в upstream-репо и в этот public template не отгружается).
...
## Единый SSOT (один файл, остальное — производные)

**Единый SSOT харнесса = `harness-workflow.yaml`** (lifecycle + toolchain + graceful
degradation + change_lifecycle + **project_management** + **local_only** + skills_index).
`harness-manifest.json` — его **производная wiring-projection**: verifier сверяет
hooks→registration→deps против неё, но истина одна. Добавил хук → строка в `harness-manifest.json`
(verifier-view) + settings.json; добавил проектный слой/lane → `harness-workflow.yaml` — не
плодить второй `*workflow*ssot*.yaml` (его нет в этом репо).

**LOCAL-ONLY (не входит в состав, не едет в mirror, не checksummed):** `.beads/` + embedded
dolt (`harness-workflow.yaml §local_only`). Каждый owner/partner/client делает `bd init` сам;
публичный template не отгружает bead-БД (gitignore `.beads/`), чтобы никто не тянул чужие
тикеты/dolt-историю. При adopt mirror **исключает** `.beads/`/`.dolt/`; verifier деградирует
gracefully когда beads нет.

## Канонические источники

- **Единый SSOT харнесса:** `harness-workflow.yaml` (lifecycle + project_management + local_only)
- Wiring-projection (verifier-view, derived): `.agents/skills/0-governance-harness-portability/harness-manifest.json`
