...
based on: [Task Master Standard](abstract://standard:task_master_standard), [Registry Standard](abstract://standard:registry_standard), [Symphony Linear Beads Orchestration Standard 4.15](abstract://standard:symphony_linear_beads_orchestration), [Speed of Trust Standard 7.1](abstract://standard:speed_of_trust_economics), [<internal-component> Knowledge Base Standard 2.8](abstract://standard:<internal-component>_knowledge_base), [Cursor Folder Structure Standard 4.6](abstract://standard:<internal-component>_cursor_folder_structure), [Hypothesis Gap Falsification Standard](abstract://standard:hypothesis_gap_falsification)
integrated: AGENTS.md (root), .agents/skills/, .agents/agents/, .beads/, <standard-ref>
version: 1.0 (draft)
...
Закрыть онбординг-разрыв для новых членов команды Heroes/<internal-component> (Милена, Крис, Аня и следующие): дать одну точку входа, которая объясняет — **как устроена работа в воркспейсе, где SSOT для каждого слоя, как пользоваться `.beads`-тикетами, скилами, субагентами, GitHub и базой знаний клиентов**, чтобы человек за 1 день дошёл до первого самостоятельного тикета без разрушения соседних потоков.

**Job to be done:** «Когда новый человек присоединяется к команде Heroes/<internal-component>, он хочет понять контур работы и начать делать тикеты, не сломав соседям git и не перепутав скилы со стандартами, чтобы за первую неделю выйти на стабильный поток задач без постоянного дёрганья Ильи».
...
Не применяется к:
- клиентским командам (для них — Стандарт 5.22 AppCraft и `4-<internal-component>-onboarding-orchestrator`);
...
|---|---|---|---|
| **Стандарты** | Каноничные правила «как мы делаем». Версионируются. | `<standard-ref>
...
| **База знаний клиента (KB)** | JTBD, виджеты, переписки, сценарии. | `[<internal-component>]/clients/all-clients/{client}/` | Не свалка артефактов — структура по 2.8. |
...
| **data-analyst** | данные | проверка gold mart перед записью в Sheets; выгрузка через <internal-component> MCP | Read, Grep, Glob, Bash + <internal-component> MCP |
...
| **telegram-delivery-sender** | доставка | форматирование + проверка читаемости (CPR) до отправки в Telegram | Telegram MCP + <internal-component> MCP |
...
6. **Runtime intent после успешного sync:** удалить строку в `.codex-memory/runtime/git-sync-intents.md`. Recurring drift → `<internal-folder>/ai.legacy.md`. RCA-worthy breakage → `ai.incidents.md`.
...
| `2.` | books · skills, projects · context | 2.7 KB JTBD, 2.8 <internal-component> KB |
...
| `5.` | <internal-component> · integrations · product heroes | 5.19 <client> regions, 5.20 user_id, 5.22 AppCraft, 5.30 cohort, 5.37 raw events |
...
- **4-** <internal-component>-специфика (виджеты, KJ, KB, snippet)
- **5-** sync, git
...
`[<internal-component>]/clients/all-clients/{client-alias}/`:
...
├── scenario-folder/exports/             # <layer>/silver parquet
├── <layer>/                              # сырые выгрузки
├── rules-registry/                      # KJ rules, attribution
├── chats/ telegram_exports/             # переписки
├── checklists/ notebooks/ sync/
```

**Эталон:** `<client>-ru/`. **Анти-эталон:** `<client>-ru/` (много рабочих artefact'ов в корне — технический долг).
...
- ❌ Класть рабочие artefact'ы в корень клиентской папки (как <client>-ru) — должны быть в `knowledge-base/{сценарий}/`.
...
- **2.8 <internal-component> Knowledge Base Standard** — структура KB.
